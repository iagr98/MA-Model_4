...
def parallel_simulation(params):
    ...
    try:
        if (experiment == "main"):
            ...
            for i, val in enumerate(Sim.h_d):
                result[f'h_d_{i}'] = val
            for i, val in enumerate(Sim.h_dpz):
                result[f'h_dpz_{i}'] = val
            
         # Schreibe das Ergebnis sofort in die CSV-Datei
        with open('simulation_results_parallel_evaluation_detail.csv', mode='a', newline='') as file:
...
if __name__ == "__main__":
    ...
    df_results.to_csv('simulation_results_parallel_evaluation_detail_1.csv', index=False)